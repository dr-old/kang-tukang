...
export default function RegisterScreen() {
    ...
    retypePassword: [
      {
        ...
        args: [password], // Pass the password value to the passwordMatch rule
      },
    ],
  };
  ...
  const signUp = useCallback(
    async (email: string, password: string) => {
      ...
      //   `🥓${password}💙`
      // );
      // console.log(email, password, digest);
      ...
      if (item?.length === 0) {
        ...
        console.log("User sign up successfully!", "success", "modal");
  ...
  const onPressSignUp = useCallback(async () => {
    try {
      ...
      console.log(`Failed to sign up: ${error?.message}`, "error", "modal");
    }
